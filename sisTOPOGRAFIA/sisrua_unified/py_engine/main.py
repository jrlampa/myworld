...
import json
import traceback
from application.dxf_orchestrator import OSMController
...
def main():
    ...
    parser.add_argument('--lon', type=float, required=False, help='Longitude')
    parser.add_argument('--radius', type=float, required=False, help='Radius in meters')
    parser.add_argument('--output', type=str, required=False, help='Output DXF filename')
    ...
    parser.add_argument('--no-preview', action='store_true', help='Skip GeoJSON preview logs')
    
    # New analysis arguments
    parser.add_argument('--mode', type=str, default='export', choices=['export', 'analyze_pad'])
    parser.add_argument('--target_z', type=float, help='Target elevation for cut/fill')
    ...
    try:
        ...
        if args.mode == 'analyze_pad':
            if not args.polygon or not args.target_z:
                raise ValueError("analyze_pad requires --polygon and --target_z")
            
            optimizer = CutFillOptimizer(json.loads(args.polygon), args.target_z)
            result = optimizer.calculate()
            print(json.dumps(result))
            sys.exit(0)

        # Default DXF Export flow
