...
from domain.osm_parser import fetch_osm_data
...
class OSMController:
    def __init__(self, lat, lon, radius, output_file, layers_config, crs, export_format='dxf', selection_mode='circle', polygon=None):
        ...
        self._analytics_res = {} # Store results for report
    ...
    def run(self):
        ...
        # 6. Advanced Services (ANEEL / Hydrology / Terrain)
        if AneelProdistRules.has_power_infrastructure(gdf):
            Logger.info("Applying ANEEL/PRODIST safety buffers...")
            prodist_gdf = AneelProdistRules.generate_faixas_servid(gdf)
            if not prodist_gdf.empty:
                dxf_gen.add_features(prodist_gdf)

        if self.layers_config.get('terrain', False):
            grid_rows = self._process_terrain(gdf, dxf_gen)
            if grid_rows and self.layers_config.get('hydrology', False):
                self._process_hydrology(grid_rows, dxf_gen)
        ...
        # Trigger Technical Report
        if self.layers_config.get('report', True):
            reporter = ReportOrchestratorUseCase(self.output_file, self.project_metadata, self.lat, self.lon)
            reporter.generate(gdf, self._analytics_res)
    ...
    def _run_audit(self, gdf):
        ...
        try:
            ...
            self._analytics_res['slope_avg'] = audit_summary.get('avg_slope', 0.0) 
    ...
    def _process_hydrology(self, grid_rows, dxf_gen):
        try:
            Logger.info("Analyzing hydrological flow...")
            z_grid = np.array([[p[2] for p in row] for row in grid_rows])
            dx = abs(grid_rows[0][1][0] - grid_rows[0][0][0])
            dy = abs(grid_rows[1][0][1] - grid_rows[0][0][1])
            
            talwegs = HydrologyService.extract_talwegs(z_grid, dx, dy)
            if talwegs:
                dxf_gen.add_drainage_lines(talwegs)
                Logger.info(f"Integrated {len(talwegs)} drainage segments.")
        except Exception as e:
            Logger.error(f"Hydrology analysis failed: {str(e)}")
    ...
    def _build_tags(self):
        ...
        if self.layers_config.get('power', True):
            tags['power'] = True
        return tags
